...
else
{
    ...
    case IDLE: 
        image_speed = 0.1;
        sprite_index = sPlayerIdleHurt;
    break;
    
    case RUN: 
        image_speed = 0.5; 
        sprite_index = sPlayerRunHurt;
    break;
    
    case JUMP:
        // Mid jump   
        if (!(place_meeting(x, y + 2, oParSolid) && v != 0) && v >= -1.0 && v <= 1.0) {  
            sprite_index = sPlayerJumpMHurt;  
       } else { 
            // Rise + fall
            if (v <= 0)  
                sprite_index = sPlayerJumpUHurt;  
            else
                sprite_index = sPlayerJumpDHurt;
        }
         
        // When against a wall   
        if (cRight || cLeft)
            sprite_index = sPlayerSlideHurt;  
    break;
}
}
